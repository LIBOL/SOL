...
int test_binary(vector<DataPoint>& dps) {
  ...
  if (dps.size() != dps2.size()) {
	  cerr << "check svm writer faileds: data point size not the same (" << dps.size() << " vs " << dps2.size() << ")!\n";
    return Status_Error;
  }
  for (size_t i = 0; i < dps.size(); ++i) {
    if (dps[i].label() != dps2[i].label()) {
		cerr<< "check svm writer failed: label of instance "<<i<<" not the same ("<<dps[i].label()<<" vs "<<dps2[i].label()<<")\n";
      return Status_Error;
    }
    for (size_t j = 0; j < dps[i].indexes().size(); ++j) {
      if (dps[i].index(j) != dps2[i].index(j)) {
		  cerr << "check svm writer failed: index " << j << " of instance " << i << " not the same\n";
        return Status_Error;
      }
      if (dps[i].feature(j) != dps2[i].feature(j)) {
		  cerr << "check svm writer failed: feature " << j << " of instance " << i << " not the same\n";
        return Status_Error;
      }
    }
  }
